...
void deleteAtPosition(tPosL p, tList *L) {  //Borra un elemento.
    ...
    for (i = p; i < L->lastPos; i++) {
        L->data[i] = L->data[i + 1];  //Aquí se va copiando el elemento siguiente del que se quiere borrar en el mismo sitio hasta que se acaba el array en el lastPos.
    }
    L->lastPos--;
}
...
bool insertItem(tItemL d, tPosL p, tList *L) {
    tPosL i;
    if (L->lastPos == MAX - 1) {
        return false;  //Si la lista esta llena n o puede introducir nada.
    } else {
        L->lastPos++;
